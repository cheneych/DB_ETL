...
public class Main {
	...
	public void addSeq(Statement stmt2,int id) throws SQLException {
		...
		intmx++;
		String start=""+intmx;
//		ResultSet rs=stmt2.executeQuery("CREATE SEQUENCE "+extractedTb[id]+"_sequence START WITH "+start+" NOCACHE NOCYCLE");
	...
	//get primary key
	public void getPK(Statement stmt,int id) throws SQLException{
		ResultSetMetaData md;
		String sPk="";
		ResultSet rs=stmt.executeQuery("select * from "+extractedTb[id]);
		md=rs.getMetaData();
		sPk=md.getColumnName(1);
		pkTb[id]=sPk;
	}
	//drop triggers
	public void dropTriggers(Statement stmt2) throws SQLException {
		ResultSet rs=stmt2.executeQuery("select 'drop trigger ' || trigger_name stmt from user_triggers");
		while (rs.next()) {
			String s=rs.getString(1);
			System.out.println(s);
			stmt2.executeQuery(s);
		}
	}
...
//		get primary key
//		for (int i=0;i<=ETL.tableNum;i++) {
//				//System.out.println(i+" table(s) get pked "+ETL.extractedTb[i]);
//				ETL.getPK(stmt,i);
//			}
		//drop trigger
//		ETL.dropTriggers(stmt2);
		//DB Extract2 
//		for (int i=0;i<=ETL.tableNum;i++) 
//		if (i>68){
